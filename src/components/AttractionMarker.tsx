...
export function AttractionMarker({ ride, telemetry }: { ride: RideWaitTime; telemetry?: RideTelemetry }) {
  ...
  return <Marker position={[ride.latitude, ride.longitude]} icon={createMapIcon(FerrisWheel, color, 'Attraction')}><Tooltip direction="top" className="map-tooltip"><strong>{ride.name}</strong><span>{ride.isOpen ? 'Open' : 'Closed'} · {ride.waitTime} min</span><span>{telemetry ? `Maintenance risk: ${telemetry.predictedFailureRisk}%` : 'Maintenance risk unavailable'}</span><small>Updated {minutesAgo(ride.lastUpdated)}</small></Tooltip></Marker>
}
